...
exports.detail = async (req, res, next) => {
    try {
        ...
        let condition = `Batch_Number__c='${id}'`;
...
exports.upsert = async (req, res, next) => {
    try{
        ...
        let uuid = '';
        let objItem = await pgRepository.findFirst(schema, object.toLowerCase(),'id,sf_id,batch_number__c', `batch_number__c = '${params.batch_number__c}'`);
        if(!objItem.success){
            objItem = await pgRepository.create(schema,object.toLowerCase(),'merchant,created_at,batch_number__c', [
                params.from__c,Date.now(),params.batch_number__c
            ]);
            uuid = objItem.id;
        }else{
            uuid = objItem.row.id;
