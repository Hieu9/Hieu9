...
exports.detail = async (req, res, next) => {
    try {
        ...
        let condition = `External_Item_Id__c='${id}'`;
...
exports.upsert = async (req, res, next) => {
    try{
        ...
        let uuid = '';
        let objItem = await pgRepository.findFirst(schema, object.toLowerCase(),'id,sf_id,external_item_id__c', `external_item_id__c = '${params.external_item_id__c}'`);
        if(!objItem.success){
            objItem = await pgRepository.create(schema,object.toLowerCase(),'merchant,created_at,external_item_id__c', [
                params.from__c,Date.now(),params.external_item_id__c
            ]);
            uuid = objItem.id;
        }else{
            uuid = objItem.row.id;
