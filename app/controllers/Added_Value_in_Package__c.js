...
const object = 'Added_Value_in_Package__c';
// list
...
exports.detail = async (req, res, next) => {
    try {
        ...
        let condition = `External_AVP_Id__c='${id}'`;
...
exports.upsert = async (req, res, next) => {
    try{
        ...
        let uuid = '';
        let objItem = await pgRepository.findFirst(schema, object.toLowerCase(),'id,sf_id,external_avp_id__c', `external_avp_id__c = '${params.external_avp_id__c}'`);
        if(!objItem.success){
            objItem = await pgRepository.create(schema,object.toLowerCase(),'merchant,created_at,external_avp_id__c', [
                params.from__c,Date.now(),params.external_avp_id__c
            ]);
            uuid = objItem.id;
        }else{
            uuid = objItem.row.id;
