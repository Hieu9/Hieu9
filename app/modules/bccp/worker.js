...
console.log('\n  🚂   Worker BCCP - SF running');

const folder_token = process.env.FOLDER_SESSION;
const PGSCHEMA = process.env.BCCP_PG_SCHEMA;
...
if (cluster.isMaster) {
	...
	for(let i=0; i < jobs_sf.length; i++){
		if(jobs_sf[i].merchant != undefined){
			console.log(jobs_sf[i].merchant);
...
async function processQueue(merchant, jobs){
	try{
		let schema = process.env.BCCP_PG_SCHEMA;
		...
		queue.process(chanel, 1, async function(job, done){
			...
			for(let i = 0; i < job.data.timeRecord; i++){
				...
				let dataSf = objLibs.funcMapFields(data, objectConfig['fieldsf'][object.toLowerCase()]);

				await libsFile.writeFileSync(filePath, dataSf, `${process.env.REDIS_KEYPREFIX}:fail:worker:${merchant}:${new Date().toISOString().slice(0,10)}:file-create:${object}:${operation}:${job.id}:${timenow}`, i);
			}
...
async function poll(merchant, schema, jobId, auth, instance_url, interval = 5 * 60 * 1000, timeout = 60 * 60 * 1000, done){
