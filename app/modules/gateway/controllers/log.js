...
exports.list = async (req, res, next) => {
    try{
        ...
        let query_str = '';
        condition ? query_str = `SELECT *, to_char(to_timestamp(created_at::numeric) at time zone 'Asia/Ho_Chi_Minh', 'YYYY-MM-DD-HH24:MI:SS') as created_at_fm FROM ${table_sql} where ${condition} ORDER BY ${orderBy} LIMIT ${limit} OFFSET ${offset}` : query_str = `SELECT *, to_char(to_timestamp(created_at::numeric) at time zone 'Asia/Ho_Chi_Minh', 'YYYY-MM-DD-HH24:MI:SS') as created_at_fm FROM ${table_sql} ORDER BY ${orderBy} LIMIT ${limit} OFFSET ${offset}`;
